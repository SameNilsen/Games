    ...
    def on_update(self, delta_time):
        ...
        for i in range(0, len(self.variabel_liste)):
            ...
            if self.particle_move_liste[i] == 1:
                if self.particle_y_liste[i] > self.height:
                    self.angle_liste[i] = 360 - self.angle_liste[i]
                if self.particle_y_liste[i] < 0:
                    self.angle_liste[i] = 360 - self.angle_liste[i]
                if self.particle_x_liste[i] > self.width:
                    self.angle_liste[i] = 180 - self.angle_liste[i]
                if self.particle_x_liste[i] < 0:
                    self.angle_liste[i] = 180 - self.angle_liste[i]
                #change = 25 - ((self.distance * 0.08) * self.variabel_liste[i])
                change = self.variabel_liste[i] * 0.007
                #print(change)
                if self.variabel_liste[i] >= 0.001:
    ...
    def on_mouse_motion(self, x, y, dx, dy):
        ...
        for i in range(0, len(self.variabel_liste)):
            ...
            if self.distance < 100:
                ...
                # variabel liste append verdi, mindre jo lengre unna, stopper når 0
                # print("angle:", (self.angle-180), "retning:", self.angle)
                self.variabel_liste[i] = 300 - self.distance
