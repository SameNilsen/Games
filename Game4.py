...
class MyGameWindow(arcade.Window):
    ...
    def on_draw(self):
        ...
        self.background.draw(0, 0, self.map_x, self.map_y)
        ...
        """
        if self.test2:
            for i in range(0, self.round_counter):
                self.sprite5 = arcade.Sprite("Sprite/Panda.png", center_x=100, center_y=100, scale=0.13)
                if self.enemy_position == 0:
                    self.sprite5.set_position((self.map_x / 2) + (50 * i), 0)
                if self.enemy_position == 1:
                    self.sprite5.set_position(-1 * (self.map_x / 2) - (50 * i), 0)
                if self.enemy_position == 2:
                    self.sprite5.set_position(0, (self.map_y / 2) + (50 * i))
                if self.enemy_position == 3:
                    self.sprite5.set_position(0, -1 * (self.map_y / 2) - (50 * i))
        ...
        if self.pause == False or self.pause_menu == True:
            arcade.draw_text(f"Level: {self.round_counter}", (self.view_left), (self.view_bottom + self.height - 20), arcade.color.DARK_RED, 20)
            if self.minutes == 0:
                arcade.draw_text(f"Tid: {int(self.seconds//1)}s", self.view_left, (self.view_bottom + self.height - 40), arcade.color.DARK_RED, 20)
            else:
                arcade.draw_text(f"Tid: {self.minutes}m:{int(self.seconds//1)}s", self.view_left, (self.view_bottom + self.height - 40), arcade.color.DARK_RED, 20)
            arcade.draw_text(f"Poeng: {self.penger}", self.view_left, (self.view_bottom + self.height - 60), arcade.color.DARK_RED, 20)
            if self.sprite5_follow:
                arcade.draw_text(f"Antall fiender: {len(self.enemy2_list)}", self.view_left, (self.view_bottom + self.height - 80), arcade.color.DARK_RED, 20)
            else:
                arcade.draw_text(f"Antall fiender: 0", self.view_left, (self.view_bottom + self.height - 80), arcade.color.DARK_RED, 20)
            arcade.draw_text(f"Posisjon: {self.player1_x//1}, {self.player1_y//1}", self.view_left + 100, (self.view_bottom + self.height - 20), arcade.color.DARK_RED, 20)
            if self.round_counter < 40:
                ...
                arcade.draw_text(f"{self.level_comment[8]}", self.view_left + 500, (self.view_bottom + self.height - 20), arcade.color.DARK_RED, 20)
            arcade.draw_text(f"Ammo: {self.ammo}", self.view_left, (self.view_bottom + self.height - 100), arcade.color.DARK_RED, 20)
        ...
        for i in range(0, len(self.ammo_pack_list), 2):
            arcade.draw_rectangle_filled(self.ammo_pack_list[i], self.ammo_pack_list[i + 1], 20, 20, arcade.color.RED)
        ...
        if self.pause == False:
            ...
            for i in range(0, len(self.ammo_pack_list), 2):
                ...
                if sqrt((ammo_avstand_x ** 2 + ammo_avstand_y ** 2)) < 50:
                    ...
                    self.ammo += self.round_counter * 5
                    break
